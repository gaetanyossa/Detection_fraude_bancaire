...
def make_prediction(features):
    ...
    probability= np.round(probability*100, 2) # cela signifie  qu'il fait des predictions a 3 chiffres apres la virgule
...
st.write("Cette application utilise un modele machine learning pour predire si un client sera en défaut de paiement")
...
sex =  st.sidebar.selectbox("Sexe" , ["Female", "Male"])
...
bill_statement_aug = st.sidebar.number_input("Relevé de facturation en août", min_value=0, value=10000)
...
previous_payment_aug = st.sidebar.number_input("Paiement précédent en août", min_value=0, value=5000)
...
if st.sidebar.button("Predire"):
    ...
    fig =  px.bar(prob_df , x= 'Categories' , y =  'Probabilité' ,text='Probabilité' , labels={'Probabilité': 'Probabilité (%)'})
    st.plotly_chart(fig)

    st.subheader("Resultat de la prediction : ")
    if prediction[0] ==1:
        st.error("Le client sera en defaut de paiement.")
    else:
        st.success("Le client ne sera pas en defaut de paiement.")